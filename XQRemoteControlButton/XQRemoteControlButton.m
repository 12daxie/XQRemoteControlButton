...
#import "XQRemoteControlButton.h"
...
#define OffSet 2.5
@interface XQRemoteControlButton()

@property (nonatomic) NSMutableArray *pathArray;
@end

@implementation XQRemoteControlButton
...
-(void)setResponsePosition:(SelectButtonPosition)position
{
    ...
    if (buttonType==ButtonTypeVPlusAndMin) {
        ...
        if (position & SelectButtonPositionBottom) {
            [_pathArray addObject:[self rectShapWithPosition:SelectButtonPositionBottom]];
        }
    }
    ...
    if (buttonType==ButtonTypeRound) {
        ...
        if (position & SelectButtonPositionBottom) {
            [_pathArray addObject:[self roundShapWithPosition:SelectButtonPositionBottom]];
        }
...
-(NSDictionary *)rectShapWithPosition:(SelectButtonPosition)position{
    ...
    switch (position) {
        ...
        case SelectButtonPositionTop:
            ...
            endAngle=0;
            break;
        case SelectButtonPositionBottom:
            center.x= CGRectGetWidth(frame)/2;
